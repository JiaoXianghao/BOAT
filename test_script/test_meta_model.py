...
class TestMetaModel:
    ...
    def test_init(self, use_t, use_warp, use_forget):
        with pytest.raises(AssertionError):
            meta_model = bohml.utils.model.MetaModel(backbone_model, use_t=use_t,
                                                     use_warp=use_warp, use_forget=use_forget)

    def test_msgd_model(self):
        meta_model = bohml.utils.model.MetaModel(backbone_model, learn_lr=True, meta_lr=0.01)
    ...
    def test_tnet_with_msgd_model(self):
        meta_model = bohml.utils.model.MetaModel(backbone_model, learn_lr=True, use_t=True)

        assert meta_model.t_modules is not None and meta_model.lr is not None

    def test_warpgrad_with_msgd_model(self):
        meta_model = bohml.utils.model.MetaModel(backbone_model, learn_lr=True, use_warp=True)

        assert meta_model.warp_modules is not None and meta_model.lr is not None

    def test_l2f_with_msgd_model(self):
        meta_model = bohml.utils.model.MetaModel(backbone_model, learn_lr=True, use_forget=True)
