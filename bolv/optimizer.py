...
from bolv.init_optimize.init import Init
...
class BOLVOptimizer(object):
    ...
    def __init__(
        ...
        super(BOLVOptimizer, self).__init__()
        ...
        if self._method == "Feature":
            ...
            if isinstance(meta_model, MetaModel):
                ...
                raise TypeError(
                    ...
                    ' be implemented with model wrapper defined by bolv.utils.model.meta_model.MetaModel'
                )
    ...
    def build_meta_solver(
        ...
        # self._meta_model = MetaModel(self._meta_model, learn_lr, inner_learning_rate, use_t,
        #                              use_warp, num_warp_layers,
        #                              use_forget, enable_inner_loop_optimizable_bn_params)
    ...
    def run_iter(
        ...
        for t_idx, (train_x, train_y, val_x, val_y) in enumerate(zip(train_data_batch, train_target_batch,
            ...
            if self._method == "Initial":
                ...
                if self._ll_method == "BVFIM":
                    ...
                    # auxiliary_opt = copy.deepcopy(self._lower_opt)
                    ...
                    # print(list(self._ll_model.parameters())[0])
                    # print(list(auxiliary_model.parameters())[0])
                    ...
                    initialize(self._ll_model)
                    # auxiliary_model_extra = higher.patch.monkeypatch(self._ll_model)
                    # auxiliary_opt_extra = higher.optim.get_diff_optim(
                    #     self._lower_opt, self._ll_model.parameters(), auxiliary_model_extra
                    # )
                    # kwargs_lower['auxiliary_model_extra'] = auxiliary_model_extra
                    # kwargs_lower['auxiliary_opt_extra'] = auxiliary_opt_extra
                else:
                    with higher.innerloop_ctx(self._ll_model, self._lower_opt,
                                              copy_initial_weights=False) as (auxiliary_model, auxiliary_opt):
                        # lower level problem optimizing
                        forward_time = time.time()
                        if self._ll_method == "Dynamic":
        ...
        #     print(val_acc)
        # batch_size = train_data_batch.shape[0]
        batch_size = 1
